...
from .generation import lzw_string_seeds
...
def parse_args():
    ...
    parser.add_argument("--seed-manifest", default=None, help="CSV containing exact symbolic seeds to replay.")
...
def generate_seed_strings(symbols, complexities, seed_count, seed, seed_manifest=None):
    if seed_manifest is not None:
        return load_seed_manifest(seed_manifest, symbols, complexities, seed_count)
    df = lzw_string_seeds(
        symbols=symbols,
...
def run(args):
    ...
    for symbols in args.symbols:
        ...
        seed_df = generate_seed_strings(symbols, args.complexities, args.seed_count, args.seed, args.seed_manifest)
                    ...
                    for layer in args.layers:
                        for unit in args.units:
                            for d_model in args.d_models:
                                hidden_size = 4 * d_model if model_name in {"Transformer", "LinearAttention", "Performer", "RWKV"} else unit
                                model = get_model(
                                    model_name,
                                    symbols,
                                    hidden_size,
...
def main():
    """Run the fixed-budget benchmark from the command line."""
    parsed = parse_args()
    configure(parsed)
    run(parsed)
